"""
these functions wrap django's url and patterns functions to allow the addition of view attributes
"""
from django.conf.urls.defaults import patterns as django_patterns, url as django_url

def url(regex, view, kwargs=None, name=None, prefix='', view_attrs=None):
    """
    this is a passthrough to Django's url function that also accepts a view_attrs keyword argument
    this allows view_attrs to be attached to Django's URL Regex resolver
    """
...
def patterns(prefix='', *args):
    """
    this is a passthrough to Django's patterns function that will attach any view_atters specified
    to the callable view objects
    """
...
def url_patterns_recurse(pattern, view_attrs):
    """
    this will handle attaching view_attrs to multiple levels of includes in urls.py
    allowing view_attrs to be specified, and override their parent view_attrs
    """
